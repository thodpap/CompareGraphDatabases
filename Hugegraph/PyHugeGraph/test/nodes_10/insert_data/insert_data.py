...
def insert_data(lines, hg):
    ...
    for line in lines[2:]:
        ...
        if vertex_1 not in myset:
            ...
            assert res_1.status_code == 201, f"{vertex_1} could not be inserted with error {res_1.response}"
        ...
        if vertex_2 not in myset:
            ...
            assert res_2.status_code == 201, f"{vertex_2} could not be inserted with error {res_2.response}"
        ...
        res_e_1 = hg.create_edge(edge_label="relationship", outv=vertex_1, inv=vertex_2, outv_label="person", inv_label="person", properties={"strong":strong})
        assert res_e_1.status_code == 201, f"{vertex_1} -> {vertex_2} edge could not be created"
        time_after_edge_1 = time.time()
        time_before_edge_2 = time.time()
        res_e_2 = hg.create_edge(edge_label="relationship", outv=vertex_2, inv=vertex_1, outv_label="person", inv_label="person", properties={"strong":strong})
        assert res_e_2.status_code == 201, f"{vertex_2} -> {vertex_1} edge could not be created"       
